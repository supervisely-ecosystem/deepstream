#!/usr/bin/env python3
"""
Simple D-FINE converter with embedded scaling - FIXED
"""
...
class SingleInputDFINEEmbeddedScaling(nn.Module):
    ...
    def forward(self, images):
        ...
        # DeepStream gives BGR 0..255, convert to PyTorch format
        images = images.float()
...
def export_to_onnx(model, output_path: str):
    ...
    torch.onnx.export(
        ...
        opset_version=16,  # FIXED: use opset 16 for grid_sampler
        verbose=False,
...
def main():
    ...
    parser.add_argument("--convert_to", type=str, choices=['onnx', 'tensorrt', 'both'], default='both')
    ...
    print("=== Simple D-FINE Converter (FIXED) ===")
    ...
    if args.convert_to in ['tensorrt', 'both']:
        if not os.path.exists(onnx_path):
            export_to_onnx(model, onnx_path)
        export_to_tensorrt(onnx_path, engine_path)
